...
def affine_forward(x, w, b):
    ...
    dim_prod = np.prod(x[0].shape)
    new_x = x.reshape(x.shape[0],dim_prod)
    out = new_x.dot(w) + b
...
def batchnorm_backward(dout, cache):
    ...
    N = 1.0 * dout.shape[0]
    df_dz = dout * cache['gamma']                                    
    du_dx = 1/N                                                      
    dv_dx = 2/N * (cache['x'] - cache['mean'])                       
    dz_dx = 1 / cache['std']                                         
    dz_du = -1 / cache['std']                                        
    dz_dv = -0.5*(cache['var']**-1.5)*(cache['x']-cache['mean'])     
    dv_du = -2/N * np.sum(cache['x'] - cache['mean'], axis=0)        

    dx = df_dz*dz_dx + np.sum(df_dz*dz_du,axis=0)*du_dx + \
         np.sum(df_dz*dz_dv,axis=0)*(dv_dx+dv_du*du_dx)
    ...
    for index in range(N):
        ...
        for i in range(0, H_pad - HH + 1, stride):
            for j in range(0, W_pad - WW + 1, stride):
                ...
                neuron += 1
        out[index] = (w_row.dot(x_col) + b.reshape(F,1)).reshape(F, H_prime, W_prime)
    ...
    for n in range(N):
        for j in range(H_prime):
            for i in range(W_prime):
                out[n, :, j, i] = np.mean(x[n, :, j * stride : j * stride + HH,
                 i * stride : i * stride + WW], axis=(-1, -2))
        ...
        for c in range(C):
            for j in range(H_prime):
                for i in range(W_prime):
                    dout1 = dout[n,c,j,i]
                    shape = (HH,WW)
                    dx[n, c, j*stride:j*stride+HH, i*stride:i*stride+WW] += distribute_value(dout1,shape)
    ###########################################################################
    #                             END OF YOUR CODE                            #
    ###########################################################################
    return dx
